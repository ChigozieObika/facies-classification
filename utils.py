...
def make_facies_log_plot(logs, facies_colors):
    ...
    ztop = logs.Depth.min()
    zbot = logs.Depth.max()  # defines the range of the log
...
def train_test_split_by_well(df, train_size):
    ...
    for i, count in enumerate(grouped_by_well['count']):
        # checks if the next well can be included in the train wells
...
def train_test_plot(df, train_df, test_df):
    ...
    plt.bar(
        x=well_counts['Well Name'],
        height=well_counts['count'],
        color=colors)
